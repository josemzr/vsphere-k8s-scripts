#!/bin/bash
# This script is used to get the SSH private key
# in a Tanzu Kubernetes cluster in vSphere with Tanzu. If you 
# set the node IP, you'll get automatically logged in.
#
# USAGE: tkg-ssh-cluster.sh $name-cluster $namespace $node_ip
# 
# Author: José Manzaneque (jmanzaneque@vmware.com)
# Dependencies: curl, jq, sshpass

SV_IP='192.168.50.129' #VIP for the Supervisor Cluster
...
# If the Node IP was set as a parameter, it will jump automatically to that VM
if [[ -z "$3" ]] 
then
loginfo "Node IP was not set as a parameter. Exiting..."
exit 2
else
IP_NODE="$3"
  # Transfer the TKC nodes SSH private key to the Supervisor Cluster Master VM
  loginfo "Node IP set as a parameter. Logging in..."
...
# Jump from the Supervisor Cluster Master VM to the node
export SSHPASS="${SV_MASTER_PASSWORD}"
sshpass -e ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -q -t root@"${SV_MASTER_IP}" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i ./tkc-ssh-privatekey vmware-system-user@"${IP_NODE}"
fi
